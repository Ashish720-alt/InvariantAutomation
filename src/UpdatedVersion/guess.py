...
class Guess:
    def __init__(self, num_var, max_num_conj, max_num_disj, op_dist, strategy: GuessStrategy, invs, **kwargs):
        """ 
        ...
        self.no_of_invariants = len(invs)
        self.__guess = self.__invariant_list(strategy, **kwargs)
    ...
    def __single_invariant(strategy, **kwargs, index):
        ret = None
        if (strategy == GuessStrategy.SMALL_CONSTANT):
            ret = lambda: self.guess_inv_small_const(kwargs["max_const"])
        elif (strategy == GuessStrategy.NEAR_CONSTANT):
            ret = lambda: self.guess_inv_near_const(kwargs["consts"], kwargs["range"])
        elif (strategy == GuessStrategy.OCTAGONAL_DOMAIN):
            ret = lambda: self.guess_inv_oct(kwargs["consts"])
        elif (strategy == GuessStrategy.OCTAGONAL_DOMAIN_EXTENDED):
            ret = lambda: self.guess_inv_oct_ext(kwargs["consts"])
        elif (strategy == GuessStrategy.MC_SMALL_CONSTANT):
            ret = lambda: self.mc_guess_inv_small_const(
                kwargs["max_const"], kwargs["prev_I"][index], kwargs["change_size_prob"], kwargs["change_value_prob_ratio"])
        elif (strategy == GuessStrategy.MC_NEAR_CONSTANT):
            ret = lambda: self.mc_guess_inv_near_const(
                kwargs["consts"], kwargs["range"], kwargs["prev_I"][index], kwargs["change_size_prob"], kwargs["change_value_prob_ratio"])
        elif (strategy == GuessStrategy.MC_OCTAGONAL_DOMAIN):
            ret = lambda: self.mc_guess_inv_oct(
                kwargs["consts"], kwargs["prev_I"][index], kwargs["change_size_prob"], kwargs["change_value_prob_ratio"])
        elif (strategy == GuessStrategy.MC_OCTAGONAL_DOMAIN_EXTENDED):
            ret = lambda: self.mc_guess_inv_oct_ext(
                kwargs["consts"], kwargs["prev_I"][index], kwargs["change_size_prob"], kwargs["change_value_prob_ratio"])
        else:
            raise Exception(
    ...
    def __invariant_list(strategy, **kwargs):
        ret = []
        for i in range(len(self.no_of_invariants)):
            ret.append(__single_invariant(strategy, **kwargs, i))
        return ret
