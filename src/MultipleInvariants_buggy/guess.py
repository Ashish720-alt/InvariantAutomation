...
class Guess:
    ...
    def __single_invariant(self, strategy, index, **kwargs ):
        ret = None
        if (strategy == GuessStrategy.SMALL_CONSTANT):
            ret = self.guess_inv_small_const(kwargs["max_const"])
        elif (strategy == GuessStrategy.NEAR_CONSTANT):
            ret = self.guess_inv_near_const(kwargs["consts"], kwargs["range"])
        elif (strategy == GuessStrategy.OCTAGONAL_DOMAIN):
            ret = self.guess_inv_oct(kwargs["consts"])
        elif (strategy == GuessStrategy.OCTAGONAL_DOMAIN_EXTENDED):
            ret = self.guess_inv_oct_ext(kwargs["consts"])
        elif (strategy == GuessStrategy.MC_SMALL_CONSTANT):
            ret = self.mc_guess_inv_small_const(
            ...
            ret = self.mc_guess_inv_near_const(
    ...
    def __invariant_list(self, strategy, **kwargs):
        ret = []
        for i in range(self.no_of_invariants):
            ret.append(self.__single_invariant(strategy, i, **kwargs))
        return ret
