...
class Cost:
    def __init__(self, repr: Repr, I_arrs):
        ...
        self.cex_list = {}
        for i in range(len(self.CHC)):
            clause = self.CHC[i]
            self.cex_list[i] = self.__get_cex( self.__get_z3_clause(clause) )

        # print(self.Is, '\n', self.cex_list) ## To remove this!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        for i in range(len(self.CHC)):
            ...
            if (clause_type == -1):
                self.cost_list[i] = self.__J_fact( self.cex_list[i]  , clause)
            elif (clause_type == 0):
                self.cost_list[i] = self.__J_inductive( self.cex_list[i]  , clause) 
            else:
                self.cost_list[i] = self.__J_query( self.cex_list[i]  , clause)    
        
        #print(self.cex_list) ## To remove this!!!!!!!!!!!! <- Gives error The cex_list adds some bogus variable examples from line 41 to 54.
        # print(self.cost_list) ## To remove this!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

        self.cost = 0.0
        for i in range(len(self.CHC)):
            clause = self.CHC[i]
            clause_type = self.__clause_type(clause)
            self.cost = self.cost + (conf.K[clause_type]*self.cost_list[i])  
    ...
    def __DNF_or_DNFz3(self, literal, primed, isarray):
        if (literal[0] == '~'):
            var = literal[1:]
            negation = 1
        else:
    ...
    # What about if DNF simplifies to True or False?!?
