...
def getlist(C, name):
    ...
    return [name, len(C.Var), len(C.P), getc(C.P), maxabsconst (C.P),  len(C.Q), getc(C.Q), maxabsconst (C.Q), len(C.B), getc(C.B), maxabsconst (C.B), 
            len(C.T), getTr_max(C.T), maxintder, minintder, minextder, maxextder, C.d, C.c  ]
...
headers = ['Filename', 'n', 'P.d' , 'P.c' , 'P.maxabs', 'Q.d', 'Q.c', 'Q.maxabs', 'B.d', 'B.c', 'B.maxabs', 'T.d', 'T.r_max', 'T.interiorderivative_max', 'T.interiorderivative_min', 
...
save_data("cdData.ods", dataexcel)
export_graphviz(regressor_d, out_file ='d_tree.dot', feature_names =(finaldata[0])[1:-4]) 
export_graphviz(regressor_c, out_file ='c_tree.dot', feature_names =(finaldata[0])[1:-4]) 