...
def convert_rhs(rhs: np.ndarray, var: list) -> str:
    ...
    rhs = [f"(* {str(rhs[i])} {var[i]})" for i in range(var_n)]
...
def convert_trans(trans: np.ndarray, var: list, var_p: list) -> str:
    ...
    if not (n == var_np + 1 and m == var_np + 1):
        raise Exception("Transition matrix shape is not correct")
    return f"""(and {" ".join([f"(= {var_p[i]} (+ {convert_rhs(trans[i][:-1], var)} {trans[i][-1]}))" for i in range(var_n)])})"""
...
def convert(i) -> str:
    # decompose input
    vars, p, b, tlist = i.Var, np.array(i.P), np.array(i.B), i.T
    q = i.Q[0][np.newaxis, :, :]
    trans, cond = [], []
    for t in tlist:
        print(t[1])
    ...
    (assert 
      (forall ({' '.join(map(lambda v: f"({v} Int)", vars + vars_p))}) 
        (and 
          (=> {P} {I_x})
          {"\n".join(map(lambda t:
                         f"(=> {convert_pred(t[1], vars)} (=> (and {I_x} {
                             B} {convert_trans(t[0], vars, vars_p)}) {I_xp}))",
                         zip(trans, cond)))}
          (=> (and (not {B}) {I_x}) {Q})
        )
      )
